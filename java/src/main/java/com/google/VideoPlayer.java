...
public class VideoPlayer {
  ...
  private String Alphabet(Video v) {
    return String.format("%s (%s) [%s]",
            v.getTitle(),
            v.getVideoId(),
            v.getTags().stream().reduce(
                    (t, s) -> t + " " + s
            ).orElse(""))
            + (flags.containsKey(v.getVideoId())
            ? (" - FLAGGED (reason: " + flags.get(v.getVideoId()) + ")") : "");
  }
  ...
  private void stopVideoIfPlaying() {
    if (videoplaying != null) {
      System.out.printf("Stopping video: %s%n", videoplaying.getTitle());
    }
    videoplaying = null;
  }
  ...
  public void pauseVideo() {
    if (videoplaying == null) {
      ...
      String videoName = videoplaying.getTitle();
      if (paused) {
    ...
    if (playList != null) {
      ...
      if (playList.videos.isEmpty()) {
        ...
        playList.videos.forEach(
                v -> System.out.println("  " + Alphabet(v))
        );
      }
    } else {
  ...
  private void searchVideosBy(Predicate<Video> function, String searchString) {
    ...
    if (videos.isEmpty()) {
      ...
      videos.forEach(x -> {
        ...
        System.out.println(number + ") " + Alphabet(x));
      });
